...
class Tree(object) :
    ...
    def find_in_tree(self,tnode,k) :
        if k < tnode.key :
            ...
            for each in tnode.children :
                self.find_in_tree(each,k)
...
""" NEED TO FIX INSERT: I MADE A MISTAKE AND IT SHOULD BE INSERTING A TNODE NOT A CNODE
...
"""
    ...
    def delete(self,tnode) :
        self.decr_key(tnode, -inf)
        self.pop()

    # merges this heap with another Fibonacci heap
    # by simply concatenating the two central lists
    def merge(self,other_heap) :
        self.min.prev.next = other_heap.min
        other_heap.min.prev.next = self.min
        self.min.prev = other_heap.min.prev
        other_heap.min.prev = self.min.prev
    ...
    def find(self,key) :
        theCNode = self.min
        for each in range(1,self.size) :
            if key < theCNode.tree.root.key :
                continue
            theTNode = theCNode.tree.find_in_tree(theCNode.tree.root,key)
            if (theTNode == None) :
...
""" Jeanette might have done this already??        
