...
class TreeNode(object): 
    ...
    def print_treenode_and_children(self,tnode,s,output_f) :
        for child in tnode.children :
            print >> output_f, (s + 'Node key:' + repr(child.key) + ' | self_url:' + child.self_url)
            if len(child.children) > 0 :
                child.print_treenode_and_children(child,self.add_indent(s))
        for neighbor in tnode.neighbors:
            print >> output_f, (s + 'Node key:' + repr(neighbor.key) + ' | self_url:' + neighbor.self_url)
...
class Tree(object) :
    ...
    def print_tree(self,output_f):
        print>>output_f, ('Tree root:' +repr(self.root.key) + ' | self_url:' + self.root.self_url)
...
class FibHeap(object) :
    ...
    def print_heap (self,output_f) :
        start_circnode = self.min
        curr_circnode = self.min
        print>> output_f, ('this is a circnode:')
